const palavras = [
    ...
    'Designer Gráfico', 'Contador', 'Chef de Cozinha', 'Fotógrafo', 'Piloto de Avião', 
    'Dentista', 'Fisioterapeuta', 'Nutricionista', 'Bibliotecário', 'Assistente Social', 
    'Biólogo', 'Químico', 'Geólogo', 'Astrônomo', 'Meteorologista', 
    'Economista', 'Publicitário', 'Tradutor', 'Ator', 'Músico', 
    'Dançarino', 'Escritor', 'Editor de Vídeo', 'Produtor de Cinema', 'Técnico de Informática', 
    'Analista de Sistemas', 'Desenvolvedor de Software', 'Engenheiro de Redes', 'Especialista em Cibersegurança', 'Administrador de Banco de Dados', 
    'Técnico em Radiologia', 'Técnico em Enfermagem', 'Técnico em Eletrônica', 'Técnico em Mecânica', 'Técnico em Automação', 
    'Técnico em Edificações', 'Técnico em Meio Ambiente', 'Técnico em Segurança do Trabalho', 'Técnico em Logística', 'Técnico em Telecomunicações'
  ];
  ...
  function escolherPalavra() {
    // Escolhe uma palavra aleatória e remove acentos
    palavra = removerAcentos(palavras[Math.floor(Math.random() * palavras.length)]).toLowerCase();
  }
  ...
  function mostrarEstado() {
    ...
    for (let i = 0; i < palavra.length; i++) {
      ...
      if (letrasAdvinhadas.includes(letra) || !/[a-zA-Z]/.test(letra)) {
        estado += letra + ' ';
      } else {
        estado += '_ ';
      }
  ...
  function guessLetter() {
    ...
    // Remove acentos da tentativa do usuário
  ...
  function allLettersGuessed() {
    return palavra.split('').every(letra => letrasAdvinhadas.includes(letra) || !/[a-zA-Z]/.test(letra));
  }
